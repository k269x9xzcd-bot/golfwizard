            ...
            <div v-if="expandedIds.has(round.id)" class="round-detail">
              ...
              <template v-if="round.game_configs?.length">
                ...
                <div class="game-recap-list">
                  <div v-for="g in gameRecapRows(round)" :key="g.id" class="game-recap-row">
                    <div class="grr-head">
                ...
                <div class="settlement-block">
                  ...
                  <template v-else-if="settlementsCache[round.id]">
                    ...
                    <div v-if="settlementsCache[round.id].ledger?.length" class="settlement-ledger">
                      <div v-for="(entry, i) in settlementsCache[round.id].ledger" :key="i" class="ledger-entry">
              ...
              <div class="round-actions">
                <button class="round-action-btn round-action-view" @click.stop="viewScorecard(round)">👁 View</button>
                <button class="round-action-btn round-action-edit" @click.stop="reopenRound(round)">✏️ Edit</button>
                ...
                <button class="round-action-btn round-action-delete" @click.stop="confirmDelete(round)">🗑️</button>
              </div>
            </div>
          </transition>
        </div>
    ...
    <div v-if="deleteTarget" class="delete-overlay" @click="deleting ? null : (deleteTarget = null)">
      <div class="delete-dialog" @click.stop>
        ...
        <div class="delete-actions">
          ...
          <button class="btn-delete-confirm" :disabled="deleting" @click="doDelete">{{ deleting ? 'Deleting…' : 'Delete' }}</button>
        </div>
      </div>
    </div>
  </div>
...
async function doShareRecap(round, event) {
  ...
  try {
    ...
    await shareHistoryRecap(round, detail)
  } catch (e) {
...
function confirmDelete(round) { deleteTarget.value = round; deleteError.value = '' }
...
async function reopenRound(round) {
  try { await roundsStore.loadRound(round.id); router.push('/scoring') }
  catch (e) { alert('Could not open round: ' + (e?.message || 'unknown error')) }
}

async function viewScorecard(round) {
  try { await roundsStore.loadRound(round.id); router.push({ path: '/scoring', query: { viewOnly: 'true' } }) }
  catch (e) { alert('Could not open scorecard: ' + (e?.message || 'unknown error')) }
}
...
function playerCount(round) { return round.round_members?.length ?? 0 }
...
function getMembersWithScores(round) {
  ...
  return members.map(m => {
    const byHole = {}
    scores.filter(s => s.member_id === m.id).forEach(s => { byHole[s.hole] = s.score ?? s.strokes })
    ...
    return { ...m, front9: front9 || null, back9: back9 || null, total: total || null }
  })
}
...
function resolveCourseForRound(round) {
  ...
  if (snap && Array.isArray(snap.par) && snap.par.length) {
    return { name: snap.name || round.course_name, par: snap.par, si: snap.si, teesData: snap.teesData, tees: snap.teesData }
  }
...
function _buildCtxForRound(round) {
  ...
  for (const s of (round.scores || [])) {
    ...
    scoreMap[s.member_id][s.hole] = s.score ?? s.strokes
  }
  return { course, tee: round.tee, members: round.round_members || [], scores: scoreMap, holesMode: round.holes_mode || '18' }
}

function gameRecapRows(round) {
  const ctx = _buildCtxForRound(round)
  if (!ctx) return []
  return (round.game_configs || []).map(g => _recapOne(ctx, g)).filter(Boolean)
}

function _recapOne(ctx, game) {
  const t = (game.type || '').toLowerCase()
  const cfg = game.config || {}
  const base = { id: game.id, icon: gameIcon(game.type), label: gameLabel(game.type), winnerLine: null, detail: null }

  try {
    if (t === 'best_ball' || t === 'bestball') {
      const r = computeBestBall(ctx, cfg); if (!r) return base
      const t1n = r.t1Name || 'T1', t2n = r.t2Name || 'T2', up = r.finalUp || 0
      ...
      const isTournament = !!cfg.tournament, pts = cfg.points || 2, ppt = r.settlement?.ppt || cfg.ppt || 0
      if (played === 0) { base.detail = `${t1n} vs ${t2n} — not started`; return base }
      if (matchOver) {
        const win = up > 0 ? t1n : t2n, lose = up > 0 ? t2n : t1n
        ...
        const leader = up > 0 ? t1n : t2n, trail = up > 0 ? t2n : t1n
    ...
    if (t === 'match' || t === 'match1v1') {
      const r = computeMatch(ctx, cfg); if (!r) return base
      const p1n = r.p1?.name || '?', p2n = r.p2?.name || '?', up = r.finalUp
      const played = (r.holeResults || []).filter(h => !h.incomplete).length
      const p1Net = r.settlement?.p1Net || 0, isTournament = !!cfg.tournament
      const pts = cfg.points || 1, ppt = r.settlement?.ppt || cfg.ppt || 0
      if (played === 0) { base.detail = `${p1n} vs ${p2n} — not started`; return base }
      if (r.matchOver) {
        const win = up > 0 ? p1n : p2n, lose = up > 0 ? p2n : p1n
        ...
        const leader = up > 0 ? p1n : p2n, trail = up > 0 ? p2n : p1n
    ...
    if (t === 'nassau') {
      const r = computeNassau(ctx, cfg); if (!r) return base
      const s = r.settlement, t1n = s.t1Name || 'T1', t2n = s.t2Name || 'T2', total = s.total || 0
      if (total === 0) { base.detail = `${t1n} vs ${t2n} — all square` }
      else {
    ...
    if (t === 'skins') {
      const r = computeSkins(ctx, cfg); if (!r) return base
      const won = (r.holeResults || []).filter(h => h.winner).length
      const topWinners = (r.settlements || []).filter(s => (s.net || 0) > 0).sort((a, b) => (b.net || 0) - (a.net || 0)).slice(0, 2)
      if (topWinners.length) base.winnerLine = topWinners.map(w => `${w.name} +$${w.net}`).join(', ')
      base.detail = won ? `${won} skin${won === 1 ? '' : 's'} paid` : 'No skins won yet'
      return base
    }
    if (t === 'dots') {
      const r = computeDots(ctx, cfg); if (!r) return base
      const top = (r.settlements || []).filter(s => (s.net || 0) > 0).sort((a, b) => (b.net || 0) - (a.net || 0))[0]
      if (top) base.winnerLine = `${top.name} +$${top.net}`
      base.detail = (r.settlements || []).map(s => `${s.name}: ${s.myDots || 0}`).join(' · ')
      return base
    }
    if (t === 'fidget') {
      const r = computeFidget(ctx, cfg); if (!r) return base
      const fidgeters = r.fidgeters || []
      const incomplete = (r.holeLog || []).some(h => h.incomplete)
      if (fidgeters.length) {
        base.detail = `Fidget: ${fidgeters.map(m => m.short_name || m.guest_name || '?').join(', ')} (owes $${r.ppp} per player)`
      } else if (incomplete) {
        const atRisk = ctx.members.filter(m => !r.hasWon?.[m.id])
        base.detail = atRisk.length ? `At risk: ${atRisk.map(m => m.short_name || '?').join(', ')}` : 'Everyone has won a hole'
      } else {
    ...
    if (t === 'snake') {
      const r = computeSnake(ctx, cfg); if (!r) return base
      base.detail = r.holderName ? `${r.holderName} holds the snake (${r.snakeCount || 0} events)` : 'No 3-putts recorded'
      return base
    }
    if (t === 'bbn') {
      const r = computeBestBallNet(ctx, cfg); if (!r) return base
      const tp = r.overallToPar
      base.detail = `Total: ${r.totalScore} (${tp === 0 ? 'E' : (tp > 0 ? `+${tp}` : `${tp}`)})`
      return base
    }
    return base
  } catch (e) { console.warn('recap error for', t, e); return base }
}
...
<style scoped>
.history-view { min-height: 100%; background: var(--gw-neutral-950); padding-bottom: calc(var(--gw-nav-height) + env(safe-area-inset-bottom) + 16px); }
.history-header { display: flex; align-items: center; gap: 12px; padding: 20px 20px 0; }
.history-title { font-family: var(--gw-font-display); font-size: 28px; color: var(--gw-text); margin: 0; flex: 1; }
.round-count { font-size: 13px; color: var(--gw-text-muted); background: rgba(255,255,255,0.08); border-radius: var(--gw-radius-full); padding: 4px 12px; }
.loading-state { display: flex; flex-direction: column; align-items: center; padding: 60px 20px; gap: 12px; }
.loading-spinner { width: 32px; height: 32px; border: 3px solid rgba(255,255,255,0.1); border-top-color: var(--gw-gold); border-radius: 50%; animation: spin 1s linear infinite; }
...
.empty-title { font-family: var(--gw-font-display); font-size: 22px; color: var(--gw-text); margin-bottom: 8px; }
.empty-sub { font-size: 14px; color: var(--gw-text-muted); margin-bottom: 24px; line-height: 1.5; }
.btn-primary { padding: 14px 28px; background: var(--gw-gold); color: #000; border: none; border-radius: var(--gw-radius-md); font-family: var(--gw-font-body); font-size: 15px; font-weight: 600; cursor: pointer; text-decoration: none; display: inline-block; -webkit-tap-highlight-color: transparent; }
.rounds-list { padding: 12px 16px; }
.month-label { font-size: 11px; font-weight: 700; letter-spacing: .08em; text-transform: uppercase; color: var(--gw-text-muted); padding: 12px 4px 6px; }
.round-card { background: var(--gw-card-bg); border-radius: var(--gw-radius-lg); margin-bottom: 10px; box-shadow: var(--gw-shadow-card); overflow: hidden; cursor: pointer; -webkit-tap-highlight-color: transparent; animation: card-in 250ms ease-out both; border-left: 3px solid var(--gw-gold); }
.round-card-header { display: flex; align-items: center; gap: 10px; padding: 14px 12px 14px 16px; }
.round-card-left { flex: 1; min-width: 0; }
.round-course-name { font-family: var(--gw-font-body); font-size: 16px; font-weight: 600; color: var(--gw-text); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.round-meta-row { display: flex; align-items: center; gap: 6px; margin-top: 3px; flex-wrap: wrap; }
...
.round-card-right { display: flex; align-items: center; gap: 8px; flex-shrink: 0; }
.round-scores-strip { display: flex; gap: 6px; }
.mini-score { text-align: center; min-width: 32px; }
.mini-name { font-size: 9px; font-weight: 600; text-transform: uppercase; letter-spacing: .04em; color: var(--gw-text-muted); }
.mini-total { font-family: var(--gw-font-mono); font-size: 16px; font-weight: 700; color: var(--gw-text); line-height: 1.2; }
.mini-total--leader { color: var(--gw-gold); }
.expand-arrow { font-size: 22px; color: rgba(240,237,224,0.3); transform: rotate(90deg); transition: transform .2s; }
.expand-arrow.expanded { transform: rotate(-90deg); }
.detail-divider { height: 1px; background: var(--gw-card-border); }
.round-detail { padding: 0 16px 16px; }
.detail-section-label { font-size: 10px; font-weight: 700; text-transform: uppercase; letter-spacing: .08em; color: var(--gw-text-muted); padding: 12px 0 8px; }
.scores-table { display: flex; flex-direction: column; gap: 2px; }
.scores-row { display: flex; align-items: center; gap: 4px; padding: 6px 0; border-bottom: 1px solid var(--gw-card-border); }
.scores-header { border-bottom: 2px solid rgba(255,255,255,0.1); padding-bottom: 4px; margin-bottom: 2px; }
.scores-header .score-player-col, .scores-header .score-num-col { font-size: 10px; font-weight: 700; text-transform: uppercase; letter-spacing: .06em; color: var(--gw-text-muted); }
.score-player-col { flex: 1; font-family: var(--gw-font-body); font-size: 14px; font-weight: 500; color: var(--gw-text); }
.score-num-col { width: 36px; text-align: center; font-family: var(--gw-font-mono); font-size: 14px; color: var(--gw-text-muted); }
.score-nine { color: var(--gw-text-muted); }
.score-total-col { width: 44px; }
.score-total { font-family: var(--gw-font-mono); font-size: 17px; font-weight: 700; color: var(--gw-text); }
.settlement-block { margin-bottom: 4px; }
.settlement-placeholder { display: flex; align-items: center; gap: 10px; padding: 12px; background: rgba(255,255,255,0.05); border-radius: var(--gw-radius-md); }
.settlement-ph-icon { font-size: 20px; }
.settlement-ph-text { font-size: 13px; color: var(--gw-text-muted); line-height: 1.4; }
.settlement-totals { display: flex; flex-direction: column; gap: 4px; margin-bottom: 10px; }
.settlement-player { display: flex; justify-content: space-between; align-items: center; padding: 8px 12px; background: rgba(255,255,255,0.04); border-radius: var(--gw-radius-sm, 8px); }
.settlement-name { font-size: 14px; font-weight: 500; color: var(--gw-text); }
.settlement-amount { font-family: var(--gw-font-mono); font-size: 15px; font-weight: 700; }
.settlement-player.positive .settlement-amount { color: #86efac; }
.settlement-player.negative .settlement-amount { color: #fca5a5; }
.settlement-ledger { display: flex; flex-direction: column; gap: 4px; }
.ledger-entry { display: flex; align-items: center; gap: 8px; padding: 8px 12px; background: rgba(212,175,55,0.06); border-radius: var(--gw-radius-sm, 8px); border-left: 3px solid var(--gw-gold); }
.ledger-from { font-size: 13px; font-weight: 600; color: #fca5a5; }
.ledger-arrow { font-size: 13px; color: var(--gw-text-muted); }
.ledger-to { font-size: 13px; font-weight: 600; color: #86efac; flex: 1; }
.ledger-amount { font-family: var(--gw-font-mono); font-size: 14px; font-weight: 700; color: var(--gw-gold); }
.settlement-even { padding: 12px; text-align: center; font-size: 13px; color: var(--gw-text-muted); background: rgba(255,255,255,0.04); border-radius: var(--gw-radius-sm, 8px); }
@keyframes card-in { from { opacity: 0; transform: translateY(8px); } to { opacity: 1; transform: translateY(0); } }
@keyframes spin { from { transform: rotate(0deg); } to { transform: rotate(360deg); } }
.expand-enter-active, .expand-leave-active { transition: opacity .2s, max-height .25s ease; overflow: hidden; max-height: 800px; }
.expand-enter-from, .expand-leave-to { opacity: 0; max-height: 0; }
.round-actions { display: flex; gap: 8px; padding-top: 12px; margin-top: 8px; border-top: 1px solid rgba(255,255,255,.06); }
.round-action-btn { flex: 1; padding: 10px 12px; border-radius: 10px; font-size: 13px; font-weight: 600; cursor: pointer; border: 1px solid rgba(255,255,255,.1); background: rgba(255,255,255,.04); color: var(--gw-text, #f0ede0); text-align: center; -webkit-tap-highlight-color: transparent; }
...
.delete-overlay { position: fixed; inset: 0; z-index: 100; background: rgba(0,0,0,.6); display: flex; align-items: center; justify-content: center; padding: 24px; }
.delete-dialog { background: var(--gw-green-800, #0d3325); border: 1px solid rgba(248,113,113,.3); border-radius: 16px; padding: 24px; max-width: 340px; width: 100%; }
.delete-title { font-size: 18px; font-weight: 800; color: #f87171; margin-bottom: 10px; }
.delete-msg { font-size: 13px; color: rgba(240,237,224,.7); line-height: 1.5; margin-bottom: 18px; }
.delete-actions { display: flex; gap: 10px; }
.btn-cancel { flex: 1; padding: 12px; border-radius: 10px; background: rgba(255,255,255,.06); border: 1px solid rgba(255,255,255,.1); color: var(--gw-text, #f0ede0); font-size: 14px; font-weight: 600; cursor: pointer; }
.btn-delete-confirm { flex: 1; padding: 12px; border-radius: 10px; background: rgba(248,113,113,.2); border: 1px solid rgba(248,113,113,.4); color: #f87171; font-size: 14px; font-weight: 700; cursor: pointer; }
.btn-delete-confirm:active { background: rgba(248,113,113,.35); }
.btn-cancel:disabled, .btn-delete-confirm:disabled { opacity: .5; cursor: wait; }
.delete-error { margin: -6px 0 14px; padding: 10px 12px; border-radius: 8px; background: rgba(239,68,68,.1); border: 1px solid rgba(239,68,68,.3); color: #f87171; font-size: 12px; line-height: 1.4; }
.game-recap-list { display: flex; flex-direction: column; gap: 6px; margin-bottom: 4px; }
.game-recap-row { padding: 10px 12px; background: rgba(255,255,255,.03); border: 1px solid rgba(255,255,255,.06); border-radius: 10px; }
.grr-head { display: flex; flex-wrap: wrap; align-items: center; gap: 6px; font-size: 13px; line-height: 1.3; }
.grr-icon { font-size: 14px; flex-shrink: 0; }
.grr-name { font-weight: 800; color: var(--gw-text, #f0ede0); }
.grr-winner { display: inline-flex; align-items: center; gap: 4px; color: #22c55e; font-weight: 700; font-size: 12px; margin-left: auto; }
.grr-star { font-size: 12px; }
.grr-detail { margin-top: 3px; font-size: 12px; color: rgba(240,237,224,.7); line-height: 1.4; }
</style>
