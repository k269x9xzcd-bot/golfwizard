  ...
  <div class="history-view">
    ...
    <div v-if="!soloRoundId" class="segment-control">
      <button
        ...
        @click="activeSegment = 'rounds'"
      >Rounds</button>
      <button
        class="segment-btn"
        :class="{ active: activeSegment === 'matches' }"
        @click="activeSegment = 'matches'"
      >Matches</button>
      <button
        class="segment-btn"
        :class="{ active: activeSegment === 'events' }"
        @click="activeSegment = 'events'"
      >Events</button>
    </div>
    ...
    <!-- ══ MATCHES segment ══ -->
    <template v-if="activeSegment === 'matches'">
      <div v-if="matchesLoading" class="loading-state">
        <div class="loading-spinner" />
        <div class="loading-text">Loading matches…</div>
      </div>
      <div v-else-if="!linkedMatchHistory.length" class="empty-state">
        <div class="empty-icon">🏌️</div>
        <div class="empty-title">No matches yet</div>
        <div class="empty-sub">Complete a cross-match to see results here.</div>
      </div>
      <div v-else class="rounds-list">
        <template v-for="group in groupedMatches" :key="group.month">
          ...
          >
            <div class="moc-top">
              ...
              <div class="moc-badge" :class="matchResultClass(match)">{{ matchResultLabel(match) }}</div>
            </div>
            <div class="moc-teams">
              <div class="moc-team">
                <div class="moc-team-label">Your Team</div>
                <div class="moc-team-names">{{ matchTeamNames(match, 'a') }}</div>
                <div class="moc-team-score" :class="{ 'score-winner': matchWinner(match) === 'a' }">
                  {{ matchTeamScore(match, 'a') }}
                </div>
              </div>
              <div class="moc-vs">vs</div>
              <div class="moc-team moc-team--right">
                <div class="moc-team-label">Opponents</div>
                <div class="moc-team-names">{{ matchTeamNames(match, 'b') }}</div>
                <div class="moc-team-score" :class="{ 'score-winner': matchWinner(match) === 'b' }">
                  {{ matchTeamScore(match, 'b') }}
                </div>
              </div>
            </div>
            <div class="moc-footer">
              <span class="moc-date">{{ formatDate(match.created_at) }}</span>
...
onMounted(async () => {
  ...
  // Support ?solo=<id> (focused recap) and ?expand=<id> (expand in list)
...
// ── Matches segment helpers ──────────────────────────────────
const groupedMatches = computed(() => {
  const map = new Map()
  for (const m of linkedMatchHistory.value) {
    const key = monthGroup(m.created_at)
    if (!map.has(key)) map.set(key, [])
    map.get(key).push(m)
  }
  return Array.from(map.entries()).map(([month, matches]) => ({ month, matches }))
})
...
function matchResultLabel(match) {
  ...
  const isHost = match.created_by === uid
  if ((isHost && w === 'a') || (!isHost && w === 'b')) return 'Win'
  return 'Loss'
}
...
.match-outcome-card:active { opacity: 0.7; }
...
.badge-win  { background: rgba(34,160,107,0.2); color: var(--gw-green-300); }
.badge-loss { background: rgba(239,68,68,0.15); color: #f87171; }
.badge-tie  { background: rgba(255,255,255,0.08); color: var(--gw-text-muted); }
