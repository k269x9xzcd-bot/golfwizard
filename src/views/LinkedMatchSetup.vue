<template>
  <div class="view lms-view">
    ...
    <div v-if="step === 'players'" class="lms-step">
      ...
      <div class="lms-hero">
        <div class="lms-hero-label">Pick your teams.</div>
        <div class="lms-hero-sub">Tap a player to assign them to Team A (you) or Team B (opposition). Max 4 per side.</div>
      </div>

      <!-- Team preview -->
      <div class="lms-teams-preview">
        <div class="lms-team-col lms-team-col--a">
          <div class="lms-team-label lms-team-label--a">⚔️ Team A</div>
          <div class="lms-team-chips">
            <span v-if="!teamASelected.length" class="lms-team-empty">Empty</span>
            <span v-for="p in teamASelected" :key="p.id" class="lms-team-chip lms-team-chip--a" @click="unassignPlayer(p)">
              {{ p.name.split(' ').slice(-1)[0] }}<span class="lms-chip-x">×</span>
            </span>
          </div>
        </div>
        <div class="lms-team-divider">vs</div>
        <div class="lms-team-col lms-team-col--b">
          <div class="lms-team-label lms-team-label--b">🏴 Team B</div>
          <div class="lms-team-chips">
            <span v-if="!foursomeBSelected.length" class="lms-team-empty">Empty</span>
            <span v-for="p in foursomeBSelected" :key="p.id" class="lms-team-chip lms-team-chip--b" @click="unassignPlayer(p)">
              {{ p.name.split(' ').slice(-1)[0] }}<span class="lms-chip-x">×</span>
            </span>
          </div>
        </div>
      ...
      <div class="lms-card">
        <div class="lms-card-header">
          ...
          <span class="lms-card-title">Roster</span>
          <span class="lms-card-badge">
            {{ teamASelected.length + foursomeBSelected.length }}/8
          </span>
        </div>
        <div class="lms-player-list">
          <div
            ...
            :key="p.id"
            class="lms-player-row lms-player-row--ab"
          >
            <div class="lms-player-info">
              <span class="lms-player-name">{{ p.name }}</span>
              <span class="lms-player-hcp" v-if="p.ghin_index != null">{{ Number(p.ghin_index).toFixed(1) }}</span>
            </div>
            <div class="lms-ab-btns">
              <button
                class="lms-ab-btn lms-ab-btn--a"
                :class="{ 'lms-ab-btn--on': teamASelected.some(s => s.id === p.id) }"
                :disabled="!teamASelected.some(s => s.id === p.id) && teamASelected.length >= 4"
                @click="toggleTeam(p, 'A')"
              >A</button>
              <button
                class="lms-ab-btn lms-ab-btn--b"
                :class="{ 'lms-ab-btn--on': foursomeBSelected.some(s => s.id === p.id) }"
                :disabled="!foursomeBSelected.some(s => s.id === p.id) && foursomeBSelected.length >= 4"
                @click="toggleTeam(p, 'B')"
              >B</button>
            </div>
          </div>
        </div>
      ...
      <div class="lms-footer">
        <button
          ...
          :disabled="foursomeBSelected.length === 0"
          @click="step = 'format'"
        >
          Format & stakes →
        </button>
...
const stepPill = computed(() => {
  if (step.value === 'players') return 'Step 1 · Pick Teams'
...
const teamASelected = ref([])
const foursomeBSelected = ref([])

function lastNameKey(p) {
  return (p.name || '').split(/\s+/).pop()?.toLowerCase() || ''
}

const sortedRoster = computed(() => {
  const favs = (rosterStore.players || []).filter(p => p.is_favorite).slice().sort((a, b) => lastNameKey(a).localeCompare(lastNameKey(b)))
  const others = (rosterStore.players || []).filter(p => !p.is_favorite).slice().sort((a, b) => lastNameKey(a).localeCompare(lastNameKey(b)))
  return [...favs, ...others]
})

function toggleTeam(player, team) {
  // If already on this team, remove (tap-to-toggle)
  if (team === 'A') {
    const idx = teamASelected.value.findIndex(p => p.id === player.id)
    if (idx >= 0) { teamASelected.value.splice(idx, 1); return }
    // Remove from B if there
    const bIdx = foursomeBSelected.value.findIndex(p => p.id === player.id)
    if (bIdx >= 0) foursomeBSelected.value.splice(bIdx, 1)
    if (teamASelected.value.length < 4) teamASelected.value.push(player)
  } else {
    const idx = foursomeBSelected.value.findIndex(p => p.id === player.id)
    if (idx >= 0) { foursomeBSelected.value.splice(idx, 1); return }
    // Remove from A if there
    const aIdx = teamASelected.value.findIndex(p => p.id === player.id)
    if (aIdx >= 0) teamASelected.value.splice(aIdx, 1)
    if (foursomeBSelected.value.length < 4) foursomeBSelected.value.push(player)
  }
}

function unassignPlayer(player) {
  teamASelected.value = teamASelected.value.filter(p => p.id !== player.id)
  foursomeBSelected.value = foursomeBSelected.value.filter(p => p.id !== player.id)
}

// Team A in the format WizardOverlay expects for lockedPlayers
const teamAAsWizardPlayers = computed(() =>
  teamASelected.value.length
    ? teamASelected.value.map(p => ({
        id: p.id,
        name: p.name,
...
.lms-player-row--ab { cursor: default; }
.lms-player-check {
...
/* A/B team buttons */
.lms-ab-btns { display: flex; gap: 5px; flex-shrink: 0; }
.lms-ab-btn {
  width: 30px; height: 30px; border-radius: 50%; border: 2px solid rgba(255,255,255,.18);
  background: transparent; color: rgba(255,255,255,.45); font-size: 12px; font-weight: 800;
  cursor: pointer; -webkit-tap-highlight-color: transparent;
  display: flex; align-items: center; justify-content: center;
  transition: background .12s, border-color .12s, color .12s;
}
.lms-ab-btn:disabled { opacity: .25; cursor: default; }
.lms-ab-btn--a.lms-ab-btn--on { background: rgba(212,175,55,.25); border-color: #d4af37; color: #d4af37; }
.lms-ab-btn--b.lms-ab-btn--on { background: rgba(99,179,237,.25); border-color: #63b3ed; color: #63b3ed; }
...
.lms-team-col { flex: 1; min-width: 0; }
.lms-team-divider {
  padding-top: 22px; font-size: 11px; font-weight: 800; letter-spacing: .05em;
  color: rgba(255,255,255,.3); flex-shrink: 0;
}
.lms-team-label { font-size: 11px; font-weight: 800; letter-spacing: .05em; margin-bottom: 6px; }
.lms-team-label--a { color: #d4af37; }
.lms-team-label--b { color: #63b3ed; }
.lms-team-chips { display: flex; flex-wrap: wrap; gap: 5px; min-height: 24px; }
.lms-team-empty { font-size: 12px; color: rgba(255,255,255,.25); line-height: 24px; }
.lms-team-chip {
...
.lms-team-chip--a { background: rgba(212,175,55,.15); border: 1px solid rgba(212,175,55,.35); color: #d4af37; }
.lms-team-chip--b { background: rgba(99,179,237,.15); border: 1px solid rgba(99,179,237,.35); color: #63b3ed; }
.lms-chip-x { opacity: .6; font-size: 11px; margin-left: 1px; }
