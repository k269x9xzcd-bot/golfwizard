    ...
    <template v-else>
      ...
      <div v-if="result?.settlement" class="lmd-settlement">
        ...
        <div v-if="result.settlement.winner" class="lmd-settlement-card lmd-settlement-card--payout">
          ...
          <span>
            Each loser pays <strong>${{ result.settlement.stakePerPlayer }}</strong> to their counterpart.
            Settle manually for this match.
          </span>
        </div>
...
const summary = computed(() => summarizeLinkedMatch(match.value, roundA.value, roundB.value, result.value))
