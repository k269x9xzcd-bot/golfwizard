...
export const useLinkedMatchesStore = defineStore('linkedMatches', () => {
  ...
  async function loadLinkedMatchDetail(matchId) {
    ...
    try {
      ...
      console.warn('[linkedMatches] raw match fetch failed, trying SJS:', e?.message)
      // Fallback to SJS only if raw fails
      try {
  ...
  /**
   * Fast raw-only round bundle loader. Skips the SJS embedded join tier entirely
   * and fires 3 parallel raw fetches (round, members, scores) with 5s timeout.
   * Falls back to SJS only if ALL raw fetches fail.
   *
   * Worst case: 5s (raw timeout) vs old path's 18.4s (SJS 10.4s + raw 8s).
   */
  async function _loadRoundBundleFast(roundId) {
    ...
    try {
      ...
      console.log(`[linkedMatches] fast bundle OK for ${roundId.slice(0, 8)}: ${round.round_members.length} members, ${round.scores.length} scores (${Date.now() - t0}ms)`)
      return round
    } catch (e) {
      console.warn(`[linkedMatches] fast bundle failed for ${roundId.slice(0, 8)} (${Date.now() - t0}ms):`, e?.message)
      // Last resort: try SJS embedded join
      return _loadRoundBundle(roundId)
    }
  }
