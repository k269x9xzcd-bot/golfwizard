...
function _crossTeamName(members) {
  ...
  // Use "Team <last name>" of the first member (captain/owner at top of scorecard)
  const captain = members[0]
  const fullName = (captain.guest_name || captain.short_name || '').trim()
  // Extract last name: split on spaces and take the last word
  const parts = fullName.split(/\s+/).filter(Boolean)
  const lastName = parts.length >= 2 ? parts[parts.length - 1] : fullName
  return lastName ? `Team ${lastName}` : ''
}
